#ifndef SERVE_SECURE_SYSCALL_H
#define SERVE_SECURE_SYSCALL_H
...
namespace sex {
    ...
    inline ssize_t process_vm_readv(pid_t pid,
                              ...
                              unsigned long flags)
    {
        return (ssize_t) __syscall6(__NR_process_vm_readv, pid, (long)local_iov, liovcnt, (long)remote_iov, riovcnt, flags);
    }
    ...
    inline ssize_t process_vm_writev(pid_t pid,
                              ...
                              unsigned long flags)
    {
        return (ssize_t) __syscall6(__NR_process_vm_writev, pid, (long)local_iov, liovcnt, (long)remote_iov, riovcnt, flags);
    }
    ...
    __attribute__((always_inline)) inline const char *strstr(const char *haystack, const char *needle) {
        size_t needle_len = sex::strlen(needle);
        if (!needle_len) {
            return haystack;
        }
        const char *p = haystack;
        while ((p = sex::strchr(p, *needle)) != NULL) {
            if (!sex::memcmp(p, needle, needle_len)) {
                return p;
            }
            p++;
        }
